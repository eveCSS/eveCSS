...
public class InHandler implements Runnable {
	...
	public InHandler( final ECP1Client ecp1client, final InputStream inputStream, final Queue< byte[] > inQueue ) {
	...
	public void run() {
		...
		final DataInputStream dataInputStream = new DataInputStream( this.inputStream );
		
		while( this.keepRunning ) {
			
			try {
				dataInputStream.readFully( headerBuffer );
				final ByteArrayInputStream byteArrayInputStream = new ByteArrayInputStream( headerBuffer );
				final DataInputStream dataInputStreamForLength = new DataInputStream( byteArrayInputStream );
