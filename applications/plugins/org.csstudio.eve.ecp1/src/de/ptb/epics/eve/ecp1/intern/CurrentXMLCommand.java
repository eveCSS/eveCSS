...
public class CurrentXMLCommand implements IECP1Command {
	...
	public CurrentXMLCommand( final String xmlName, final String xmlAuthor, final byte[] xmlData ) {
		if( xmlName == null ) {
			throw new IllegalArgumentException( "The paramter 'xmlName' must not be null!" );
		}
		if( xmlAuthor == null ) {
			throw new IllegalArgumentException( "The paramter 'xmlAuthor' must not be null!" );
		}
		if( xmlData == null ) {
			throw new IllegalArgumentException( "The paramter 'xmlData' must not be null!" );
		}
	...
	public CurrentXMLCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		if( byteArray == null ) {
		...
		if( commandTypeID != CurrentXMLCommand.COMMAND_TYPE_ID ) {
			throw new WrongTypeIdException( byteArray, commandTypeID, CurrentXMLCommand.COMMAND_TYPE_ID );
		}
		...
		if( xmlNameLength != 0xffffffff ) {
			...
			this.xmlName = new String( xmlNameBuffer, IECP1Command.STRING_ENCODING );
		} else {
			this.xmlName = "";
		}
		...
		if( xmlAuthorLength != 0xffffffff ) {
			...
			this.xmlAuthor = new String( xmlAuthorBuffer, IECP1Command.STRING_ENCODING );
		} else {
			this.xmlAuthor = "";
		}
	...
	public byte[] getByteArray() throws IOException {
		...
		final byte[] xmlNameBuffer = this.xmlName.getBytes( IECP1Command.STRING_ENCODING );
		final byte[] xmlAuthorBuffer = this.xmlAuthor.getBytes( IECP1Command.STRING_ENCODING );
		
		dataOutputStream.writeInt( 12 + xmlNameBuffer.length + xmlAuthorBuffer.length + xmlData.length );
