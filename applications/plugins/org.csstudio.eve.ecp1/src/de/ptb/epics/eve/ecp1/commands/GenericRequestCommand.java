...
public class GenericRequestCommand implements IECP1Command {
	...
	public GenericRequestCommand(final int requestId,
			final RequestType requestType, final String requestText) {
		if (requestText == null) {
			throw new IllegalArgumentException(
					"The parameter 'requestText' must not be null!");
		}
	...
	public GenericRequestCommand(final byte[] byteArray) throws IOException,
		...
		if (lengthOfRequestText != 0xffffffff) {
			...
			this.requestText = new String(requestTextBuffer,
	...
	public byte[] getByteArray() throws IOException {
		...
		dataOutputStream
				.writeInt((this.requestText.length() != 0) ? 12 + requestTextBuffer.length
						: 12);
