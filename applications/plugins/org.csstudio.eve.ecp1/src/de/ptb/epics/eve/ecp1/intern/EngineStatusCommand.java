...
public class EngineStatusCommand implements IECP1Command {
	...
	public EngineStatusCommand( final int generalTimeStamp, final int nanoseconds, final EngineStatus engineStatus, final boolean autoplay, final String xmlName ) {
	...
	public EngineStatusCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		if( byteArray == null ) {
		...
		if( autoplayByte == 0 ) {
			...
			throw new BrokenByteArrayException( byteArray, "Wrong value for autoplay. Expected 0 or 1, got " + autoplay + "!" );
		}
		
		this.engineStatus = EngineStatus.byteToEngineStatus( dataInputStream.readByte() );
