...
public class AddToPlayListCommand implements IECP1Command {
	...
	public AddToPlayListCommand( final String name, final String author, final byte[] xmlData ) {
		if( name == null ) {
			throw new IllegalArgumentException( "The paramter 'name' must not be null!" );
		}
		if( author == null ) {
			throw new IllegalArgumentException( "The paramter 'author' must not be null!" );
		}
		if( xmlData == null ) {
	...
	public AddToPlayListCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		if( byteArray == null ) {
		...
		if( commandTypeID != AddToPlayListCommand.COMMAND_TYPE_ID ) {
			throw new WrongTypeIdException( byteArray, commandTypeID, AddToPlayListCommand.COMMAND_TYPE_ID );
		}
		...
		if( authorLength != 0xffffffff ) {
			...
			this.author = new String( authorByteArray, IECP1Command.STRING_ENCODING );
		} else {
			this.author = "";
		}
	...
	public byte[] getByteArray() throws IOException {
		...
		final byte[] nameByteBuffer = this.name.getBytes( IECP1Command.STRING_ENCODING );
		final byte[] authorByteBuffer = this.author.getBytes( IECP1Command.STRING_ENCODING );
		int length = 12;
