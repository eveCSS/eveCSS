...
public class CancelRequestCommand implements IECP1Command {

	public static final char COMMAND_TYPE_ID = 0x0109;
	
	private int requestId;
	
	public CancelRequestCommand( final int requestId ) {
		this.requestId = requestId;
	}
	
	public CancelRequestCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		if( byteArray == null ) {
		...
		if( commandTypeID != CancelRequestCommand.COMMAND_TYPE_ID ) {
			throw new WrongTypeIdException( byteArray, commandTypeID, CancelRequestCommand.COMMAND_TYPE_ID );
		}
