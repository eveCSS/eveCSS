...
* [Bug Fix] adding channel snapshot scan modules (or converting them via fill options) and removing a channel (from a channel snapshot module) does now take a reasonable time (again, and does not scale with the number of devices)
