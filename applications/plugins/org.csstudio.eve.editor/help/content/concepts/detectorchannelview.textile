h1(eveTopic). Detector Channel View

p. The Detector Channel View displays (and allows to modify) properties of a channel. The channel being shown is the one selected in the "Scan Module View":scanmodule.html. 
...
p. *Max. Attempts:* If the tolerance check failed the given value amount of times, the check is stopped an the next values are used for the average calculation.
...
p. *Trigger Interval:* The interval/rate measurements are taken.
