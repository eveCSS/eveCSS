...
p. When monitor events are combined (one in the chain, one in the scan module) a redo occurs if at least one of their last sent values was @true@. Only when both last sent @false@ the scan continues normally.