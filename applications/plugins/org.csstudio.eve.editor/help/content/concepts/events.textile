...
p. Events are a way to affect scans, e.g. by pausing it if a condition is met. Three types (describing their source) of events exist:
...
p. +Note+: Each event can continue the scan (stop the pause) regardless of which event paused it ("learn more":#combine on combining events).
...
p. Chains and scan modules have +independent+ pause states, i.e. both scan module and chain states must be _running_ in order to continue a scan.
