...
p. Scan modules are self-contained units which are partitioned into three stages, a prescan phase, a main phase and a postscan phase:
...
*Note* that a scan modules prescan and postscan phases are only processed once (not at each iteration of the nested module(s)).
