...
p. The Deviation of the first two measured values must be less or equal then __Max. Deviation (%)__ to be valid. If that constraints ist not fulfilled, the first measured value is discarded and a new value is taken until the condition is met.

h2(eveTopic). Minimum:

p. If the first measured value is ==&ge;== __Minimum__ a tolerance check is conducted. 

h2(eveTopic). Max. Attempts:

p. If the tolerance check failed __Max. Attempts__ times, the check is stopped an the next values are used for the average calculation.

h2(eveTopic). Normalize Channel:

p. The __Normalize Channel__ is the channel on wich the __Detector Channel__ should be normalized. The calculation of the deviation to be based on the normalized value.
...
h2(eveTopic). Redo Event

p. In the __Redo__ Tab you choose which Redo Events you set for the detector channel. The description of events an which event types are available you found "here":events.html
