...
public class MeasurementDataCommand implements IECP1Command {
	...
	public MeasurementDataCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		...
		if( commandTypeID != MeasurementDataCommand.COMMAND_TYPE_ID ) {
			throw new WrongTypeIdException( byteArray, commandTypeID, MeasurementDataCommand.COMMAND_TYPE_ID );
		}
		...
		dataInputStream.readChar();
		dataInputStream.readByte();
		this.dataModifier = DataModifier.byteTotDataModifyer( dataInputStream.readByte() );
		this.epicsSeverity = EpicsSeverity.byteToEpicsSeverity( dataInputStream.readByte() );
		this.epicsStatus = EpicsStatus.byteToEpicsStatus( dataInputStream.readByte() );
		dataInputStream.readByte();
		this.acquisitionStatus = AcquisitionStatus.byteToAcquisitionStatus( dataInputStream.readByte() );
	...
	public byte[] getByteArray() throws IOException {
		...
		dataOutputStream.writeChar( 0 );
		dataOutputStream.writeByte( 0 );
		dataOutputStream.writeByte( DataModifier.dataModifyerToByte( this.dataModifier ) );
		
		dataOutputStream.writeByte( EpicsSeverity.epicsSeverityToByte( this.epicsSeverity ) );
		dataOutputStream.writeByte( EpicsStatus.epicsStatusToByte( this.epicsStatus ) );
		dataOutputStream.writeByte( 0 );
		dataOutputStream.writeByte( AcquisitionStatus.acquisitionStatusToByte( this.acquisitionStatus ) );
		...
		case INT16:
			for( int i = 0; i < this.values.size(); ++i ) {
				dataOutputStream.writeChar( ((List< Character >)this.values).get( i ) );
			}
			break;
		...
		case DOUBLE:
			for( int i = 0; i < this.values.size(); ++i ) {
				dataOutputStream.writeDouble( ((List< Double >)this.values).get( i ) );
			}
			break;
