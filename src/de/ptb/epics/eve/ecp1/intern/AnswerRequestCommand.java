...
public class AnswerRequestCommand implements IECP1Command {
	...
	public AnswerRequestCommand( final int requestId, final RequestType requestType, final boolean booleanValue ) {
	...
	public AnswerRequestCommand( final int requestId, final RequestType requestType, final String textValue ) {
		this.requestId = requestId;
		this.requestType = requestType;
		this.textBuffer = textValue;
	}
	
	public AnswerRequestCommand( final byte[] byteArray ) throws IOException, AbstractRestoreECP1CommandException {
		if( byteArray == null ) {
		...
		if( commandTypeID != AnswerRequestCommand.COMMAND_TYPE_ID ) {
			throw new WrongTypeIdException( byteArray, commandTypeID, AnswerRequestCommand.COMMAND_TYPE_ID );
		}
		...
		switch( this.requestType ) {
			...
			case ERROR_TEXT:
				...
				if( lenghtOfText != 0xffffffff ) {
					...
					this.textBuffer = new String( byteBuffer, IECP1Command.STRING_ENCODING );
				}
		}
	}
	...
	public byte[] getByteArray() throws IOException {
		...
		switch( this.requestType ) {
			...
			case TEXT:
			case ERROR_TEXT:
				byte[] buffer = this.textBuffer.getBytes( IECP1Command.STRING_ENCODING );
	...
	public int getIntegerValue() {
		return (this.requestType == RequestType.INT32)?this.integerBuffer:Integer.MIN_VALUE;
	}
	...
	public float getFloatValue() {
		return (this.requestType == RequestType.FLOAT32)?this.floatBuffer:Float.MIN_VALUE;
	}
	...
	public String getErrorText() {
		return this.requestType == RequestType.ERROR_TEXT?this.textBuffer:null;
	}
