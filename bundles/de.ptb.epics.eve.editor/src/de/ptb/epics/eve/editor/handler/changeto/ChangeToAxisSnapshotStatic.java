...
import de.ptb.epics.eve.editor.jobs.changesmtype.AxisSnapshotStatic;

/**
 * @author Marcus Michalsky
 * @since 1.31
 */
public class ChangeToAxisSnapshotStatic extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (element instanceof ScanModuleEditPart) {
			...
			UIJob axisSnapshotStaticJob = 
					new AxisSnapshotStatic(scanModule, measuringStation);
			axisSnapshotStaticJob.setUser(true);
			axisSnapshotStaticJob.addJobChangeListener(
					new ReselectScanModuleJobChangeListener(selectionProvider, 
							((ScanModuleEditPart)element)));
			axisSnapshotStaticJob.schedule();
		} else {
