...
import de.ptb.epics.eve.editor.jobs.changesmtype.ChannelSnapshotDynamic;

/**
 * @author Marcus Michalsky
 * @since 1.31
 */
public class ChangeToChannelSnapshotDynamic extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (element instanceof ScanModuleEditPart) {
			...
			UIJob channelSnapshotDynamicJob = new ChannelSnapshotDynamic(scanModule);
			channelSnapshotDynamicJob.setUser(true);
			channelSnapshotDynamicJob.addJobChangeListener(
					new ReselectScanModuleJobChangeListener(selectionProvider, 
							((ScanModuleEditPart)element)));
			channelSnapshotDynamicJob.schedule();
		} else {
