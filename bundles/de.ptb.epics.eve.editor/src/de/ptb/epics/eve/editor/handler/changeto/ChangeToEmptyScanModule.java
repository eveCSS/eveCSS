...
import de.ptb.epics.eve.editor.jobs.changesmtype.EmptyScanModule;

/**
 * @author Marcus Michalsky
 * @since 1.31
 */
public class ChangeToEmptyScanModule extends AbstractHandler {
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (element instanceof ScanModuleEditPart) {
			...
			UIJob emptyScanModuleJob = new EmptyScanModule(scanModule);
			emptyScanModuleJob.setUser(true);
			emptyScanModuleJob.addJobChangeListener(
					new ReselectScanModuleJobChangeListener(selectionProvider, 
							((ScanModuleEditPart)element)));
			emptyScanModuleJob.schedule();
		} else {
