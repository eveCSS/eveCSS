 ...
 * @since 1.28
 */
public class AxesModifierInvertY2 extends AbstractHandler {
	private static final Logger LOGGER = Logger.getLogger(
			AxesModifierInvertY2.class.getName());
	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (collectors.size() < 2) {
			throw new ExecutionException("plot window has only one trace");
		}
		
		if (oldState) {
			collectors.get(1).setYAxisModifier(YAxisModifier.NONE);
		} else {
			collectors.get(1).setYAxisModifier(YAxisModifier.INVERSE);
		}
		return null;
	}
